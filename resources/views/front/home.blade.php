                        ...
                        <div class="col-md-3">
                            <div class="card product-card">
                                <div class="product-image position-relative">
                                    <a href="{{ route('front.product',$fProduct->slug) }}" class="product-img">
                                    ...
                                    <a onclick="addToWishList({{ $fProduct->id }})" class="whishlist" href="javascript:void(0);"><i class="far fa-heart"></i></a>                            

                                    <div class="product-action">
                                        @if ($fProduct->track_qty == 'Yes')
                                            @if ($fProduct->qty > 0)
                                ...
                                <div class="card-body text-center" style="background: beige">
                                    <a class="h6 link" href="{{ route('front.product',$fProduct->slug) }}">{{ $fProduct->title }}</a>
                                    <div class="price mt-2">
                                        <span class="h5"><strong>&#8377;{{ number_format($fProduct->price,2) }} </strong></span>
                                        @if ($fProduct->compare_price > 0)
                                            <span class="h6 text-underline"><del>&#8377;{{ number_format($fProduct->compare_price,2) }}</del></span>    
                                        @endif                                                
                                    </div>
                                </div>                        
                            </div>                                               
            ...
            <div class="row pb-3">
                @if ($latestProducts->isNotEmpty())
                    @foreach ($latestProducts as $lProduct)
                        @php
                            $productImage = $lProduct->product_images->first();
                        @endphp
                        <div class="col-md-3">
                            <div class="card product-card">
                                <div class="product-image position-relative">
                                    <a href="{{ route('front.product',$lProduct->slug) }}" class="product-img">
                                        {{-- <img class="card-img-top" src="{{ asset('front-assets/images/product-1.jpg') }}" alt=""> --}}
                                        @if (!empty($productImage->image))
                                            <img class="card-img-top" src="{{ asset('uploads/product/small/'.$productImage->image) }}" />
                                        @else
                                    ...
                                    <a onclick="addToWishList({{ $lProduct->id }})" class="whishlist" href="javascript:void(0);"><i class="far fa-heart"></i></a>                            
                                ...
                                <div class="card-body text-center" style="background: beige">
                                    <a class="h6 link" href="{{ route('front.product',$lProduct->slug) }}">{{ $lProduct->title }}</a>
                                    <div class="price mt-2">
                                        <span class="h5"><strong>&#8377;{{ number_format($lProduct->price,2) }} </strong></span>
                                        @if ($lProduct->compare_price > 0)
                                            <span class="h6 text-underline"><del>&#8377;{{ number_format($lProduct->compare_price,2) }}</del></span>    
                                        @endif  
                                    </div>
                                </div>                        
                            </div>                                               
